...
import { BookOpen, Check, Send, Star, Upload, Users, X } from "lucide-react";
...
import { getDoc, doc, query, collection, where, getDocs, setDoc, addDoc, updateDoc } from "firebase/firestore";
...
const MentorDashboard = ({ MENTOR_ID }: { MENTOR_ID: any }) => {
  console.log("Current MENTOR_ID:", MENTOR_ID);
  ...
  useEffect(() => {
    const loadMentorData = async () => {
      try {
        console.log("Loading mentor data for ID:", MENTOR_ID);
        ...
        // We don't need mentorships state anymore since we're using subscriptions
        setMentorships([]); // You can eventually remove this state entirely
  
        // Calculate statistics
        const activeCount = subscriptionsData.filter(s => s.status === 'ACCEPTED').length;
        const completedCount = subscriptionsData.filter(s => s.status === 'COMPLETED').length;
  ...
  const handleOpenChat = async (subscription: Subscription) => {
    setCurrentChat(subscription);
    setChatOpen(true);
  
    try {
      const messagesQuery = query(
        collection(db, 'messages'),
        where('subscriptionId', '==', subscription.id) // Update this field name if needed
      );
  ...
  const handleCompleteSubscription = async (subscriptionId: string) => {
    try {
      const subscriptionRef = doc(db, 'subscriptions', subscriptionId);
      await updateDoc(subscriptionRef, {
        status: 'COMPLETED',
      ...
      setSubscriptions(prevSubscriptions =>
        prevSubscriptions.map(sub =>
          sub.id === subscriptionId 
            ? { ...sub, status: 'COMPLETED', endDate: new Date().toISOString() } 
            : sub
        )
      );
  
      alert('Mentoría marcada como completada exitosamente');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="relative w-full bg-[#1e3a8a] z-10">
      <header className="w-full text-white py-6 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            ...
            <Button 
              variant="outline" 
              className="bg-white text-[#1e3a8a] hover:bg-white/90 rounded-full px-6"
              onClick={async () => {
                await auth.signOut();
                setTimeout(async () => {
                  await userLogOut();
      ...
      <div className="container mx-auto px-6">
                        ...
                        <div className="space-y-4">
                            ...
                            .map((subscription) => (
                                ...
                                <div>
                                    <h4 className="font-medium text-gray-900">{subscription.studentName}</h4>
                                    <p className="text-sm text-gray-500">
                                    Creado: {new Date(subscription.createdAt).toLocaleDateString()}
                                    </p>
                                    <p className="text-sm text-gray-500">
                                    Tarifa: ${subscription.hourlyRate}/hora
                                    </p>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    onClick={() => handleOpenChat(subscription)}
                                    >
                                    ...
                                    onClick={() => handleCompleteSubscription(subscription.id)}
                                    >
        ...
        {/* Dialog para detalles de subscripción */}
